...
Feature: To check for all the hyperlinks present in Bugzilla main page
  ...
  Background: 
    Given User should be at Bugzilla Home page "http://localhost:5001/"

  Scenario Outline: To check for links
    # Given User should be at Home page "http://localhost:5001/"
    When I click on the link name "<linkText>"
    Then I  assert for the title value "<titleValue>"
    And I close the driver session
